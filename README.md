## Инструкция по установке корпоратвного мессенджера для общения и командной работы UnicChat

###### версия free-1.0.0, версия документа 1.1
### Оглавление 
<!-- TOC -->
  * [Инструкция по установке корпоратвного мессенджера для общения и командной работы UnicChat](#инструкция-по-установке-корпоратвного-мессенджера-для-общения-и-командной-работы-unicchat)
    ...
    * [Шаг Х. Получение сертификата Let's Encrypt](#шаг-х-получение-сертификата-lets-encrypt)
    * [Шаг Х. Открыть доступы до внутренних ресурсов](#шаг-х-открыть-доступы-до-внутренних-ресурсов)
      * [Входящие соединения на стороне сервера UnicChat:](#входящие-соединения-на-стороне-сервера-unicchat-)
      * [Исходящие соединения на стороне сервера UnicChat:](#исходящие-соединения-на-стороне-сервера-unicchat-)
    * [Шаг 7. Создание пользователя администратора](#шаг-7-создание-пользователя-администратора)
...
___
![](./assets/1vm-install-guide-1.png "Архитектура установки")
...
Для работы видео- и аудиозвонков необходимы протоколы STUN и TURN: входящие соединения на порты 3478/TCP и 3478/UDP,
а также входящий и исходящий трафик UDP по портам 1024+ (RTP-трафик).
...
2. Теперь в командной строке внутри контейнера выполним подключение c помощью `mongosh`
...
4. Теперь можно приступить к настройке сервера и создания первого пользователя (перейдите на [Шаг 7. Создание пользователя администратора](#шаг-7-создание-пользователя-администратора)) или  
   настройте прокси сервер и получите сертификат HTTPS, для этого перейдите на [Шаг 6. Настройка nginx как Proxy](#шаг-6-настройка-nginx-как-proxy)
...
### Шаг Х. Получение сертификата Let's Encrypt

### Шаг Х. Открыть доступы до внутренних ресурсов
...
* Открыть доступ до всех внутренних ресурсов: LDAP, SMTP, DNS.
