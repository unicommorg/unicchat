...
- **Nginx с SSL является обязательным компонентом** для production установки (настраивается отдельно, см. раздел 2.6)
...
<!-- TOC --><a name="-nginx"></a>
## 2.6. Настройка Nginx с SSL (обязательно)

Nginx с SSL сертификатами от Let's Encrypt является обязательным компонентом для работы UnicChat в production. Выполните следующие шаги:
...
3. **Проверьте файл `unicchat_config.txt`** (должен быть в корне проекта):
   ```bash
   cd ..
   ...
   ```
   **Важно:** EMAIL обязателен для получения SSL сертификата. Добавьте его в файл, если отсутствует.
...
**Важно:** Nginx с SSL является обязательным компонентом для production установки UnicChat. После настройки UnicChat будет доступен по адресу `https://ваш-домен`.
