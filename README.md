
<!-- TOC --><a name="-unicchat"></a>
# Инструкция по установке корпоративного мессенджера для общения и командной работы UnicChat

<!-- TOC --><a name="-6-2170-17"></a>
###### Версия 6-2.1.70, версия документа 1.7
<!-- TOC start (generated with https://github.com/derlin/bitdowntoc) -->

   * [Оглавление](#)
   * [Архитектура установки](#-)
      [Установка на 1-м сервере](#-1-)
      [Установка на 2-x серверах](#-2-x-)
   * [1. Подготовка окружения](#-1--1)
       * [Требования к конфигурации до 50 пользователей. Приложение и БД устанавливаются на 1-й виртуальной машине](#-50-1-)
  
         * [Конфигурация виртуальной машины](#--1)
  
         * [Требования для установки](#--2)
   * [2. Установка UnicChat](#-2-unicchat)
     * [2.1. Скачайте репозиторий](#21-clone)
     * [2.2. Переключитесь на root-пользователя](#22-unicchatsh)
     * [2.3. Дайте скрипту права на выполнение](#23-)
     * [2.4. Запустите скрипт установки](#24-)
     * [2.5. Выполните шаги в меню](#25-)
     * [2.6. Настройка Nginx с SSL (опционально)](#-nginx)
     * [Важные примечания](#--3)
     * [Устранение проблем](#--4)
   * [3. Создание пользователя-администратора](#-3-)
   * [4. Карта сетевых взаимодействий сервера](#-4-)
         - [Входящие соединения на стороне сервера UnicChat:](#-unicchat-1)
         - [Исходящие соединения на стороне сервера UnicChat:](#-unicchat-2)
   * [Частые проблемы при установке](#--5)
   * [Клиентские приложения](#--6)

<!-- TOC end -->





<!-- TOC --><a name=""></a>
## Оглавление
<!-- TOC -->

<!-- TOC -->

<!-- TOC --><a name="-"></a>
## Архитектура установки

<!-- TOC --><a name="-1-"></a>
#### Установка на 1-м сервере
![](./assets/1vm-unicchat-install-scheme.jpg "Архитектура установки на 1-м сервере")
<!-- TOC --><a name="-2-x-"></a>
#### Установка на 2-x серверах
![](./assets/2vm-unicchat-install-scheme.jpg "Архитектура установки на 2-х серверах")

<!-- TOC --><a name="-1--1"></a>
## 1. Подготовка окружения

<!-- TOC --><a name="-50-1-"></a>
#### Требования к конфигурации до 50 пользователей. Приложение и БД устанавливаются на 1-й виртуальной машине

<!-- TOC --><a name="--1"></a>
##### Конфигурация виртуальной машины
```
CPU 4 cores 1.7ghz, с набором инструкций FMA3, SSE4.2, AVX 2.0;
RAM 16 Gb;
250 Gb HDD\SSD;
```
Для ОС Ubuntu 20+ предлагаем воспользоваться нашими краткими инструкциями. Для других ОС воспользуйтесь инструкциями, размещенными в сети Интернет.
<!-- TOC --><a name="--2"></a>
##### Требования для установки
- Сервер на Linux (рекомендуется Ubuntu 20+/Debian)
- Доступ с правами root или через sudo
- Доменное имя, привязанное к IP-адресу вашего сервера (или используйте `localhost` для локальной установки)

<!-- TOC --><a name="-2-unicchat"></a>
## 2. Установка UnicChat

<!-- TOC --><a name="21-clone"></a>
### 2.1. Скачайте репозиторий

Клонируйте репозиторий UnicChat:

```bash
git clone https://github.com/unicommorg/unicchat.git
cd unicchat
```

<!-- TOC --><a name="22-unicchatsh"></a>
### 2.2. Переключитесь на root-пользователя
```bash
sudo su
```

<!-- TOC --><a name="23-"></a>
### 2.3. Дайте скрипту права на выполнение
```bash
chmod +x unicchat.sh
```

<!-- TOC --><a name="24-"></a>
### 2.4. Запустите скрипт установки
```bash
./unicchat.sh
```

<!-- TOC --><a name="25-"></a>
### 2.5. Выполните шаги в меню

Можете выбрать полностью автоматизированную установку 9️⃣9️⃣

| Шаг | Действие | Описание |
|-----|----------|----------|
| 1️⃣ | Установка зависимостей | Docker, docker-compose, Git |
| 2️⃣ | Проверка AVX | Определение совместимой версии MongoDB |
| 3️⃣ | Настройка домена | Конфигурация доменного имени и проверка DNS |
| 4️⃣ | Создание Docker сети | Создание сети unicchat-network |
| 5️⃣ | Подготовка env-файлов | Создание необходимых переменных окружения и credentials |
| 6️⃣ | Аутентификация в Yandex Registry | Вход в реестр контейнеров |
| 7️⃣ | Запуск UnicChat | Запуск всех сервисов через Docker |
| 8️⃣ | Настройка MongoDB пользователей | Создание пользователей для logger и vault сервисов |
| 9️⃣ | Настройка Vault secrets | Настройка секретов для KBT сервиса |
| 🔟 | Перезапуск всех сервисов | Полный перезапуск всех контейнеров UnicChat |
| 9️⃣9️⃣| Автоматизированная установка | Исполняет все шаги автоматически |
| 1️⃣0️⃣0️⃣| Очистка | Удаление контейнеров, volumes и images


<!-- TOC --><a name="--3"></a>
## Важные примечания
- Доменное имя должно иметь A-запись в DNS перед запуском сервисов
- Для процессоров без AVX используется MongoDB 4.4 (определяется автоматически на шаге 2)
- После завершения установки UnicChat будет доступен по адресу: `https://ваш-домен` (или `http://localhost:8080` для локальной установки)
- Credentials сохраняются в файл `unicchat_creds.txt` для повторного использования при переустановке

<!-- TOC --><a name="--4"></a>
## Устранение проблем
- Если скрипт не видит файл `unicchat_config.txt`, проверьте:
  - Файл должен находиться в той же директории, что и скрипт
  - Формат файла: `DOMAIN=ваш-домен` на отдельной строке
- Для перезапуска сервисов используйте пункт меню [10] или команду:
  ```bash
  cd single-server-install
  docker-compose -f unicchat.yml down
  docker-compose -f unicchat.yml up -d
  ```
- Для проверки логов контейнеров используйте:
  ```bash
  docker logs <имя_контейнера>
  ```
- При проблемах с MongoDB проверьте, что пользователи созданы корректно (пункт меню [8])

<!-- TOC --><a name="-nginx"></a>
## 2.6. Настройка Nginx с SSL (опционально)

Если вы хотите использовать Nginx в качестве reverse proxy с SSL сертификатами от Let's Encrypt, выполните следующие шаги:

### Требования
- Docker и Docker Compose установлены
- Доменное имя, указывающее на IP-адрес вашего сервера
- Открытые порты 80 и 443 в firewall

### Быстрый старт

1. **Перейдите в директорию nginx:**
   ```bash
   cd nginx
   ```

2. **Выдайте права на выполнение скрипту:**
   ```bash
   chmod +x generate_ssl.sh
   ```

3. **Создайте файл с настройками домена** (в корне проекта, на уровень выше `nginx/`):
   ```bash
   cd ..
   cat > domain.usr.txt << EOF
   DOMAIN=yourdomain.com
   EMAIL=your@email.com
   EOF
   ```
   Замените `yourdomain.com` и `your@email.com` на ваши реальные данные.

4. **Настройте upstream сервера** в `nginx/config/nginx.conf.template`:
   ```nginx
   upstream internal {
       server 127.0.0.1:8080;  # Адрес вашего UnicChat приложения
   }
   ```

5. **Запустите скрипт настройки:**
   ```bash
   cd nginx
   sudo ./generate_ssl.sh
   ```

6. **В меню выберите опцию [1] 🔐 Генерация SSL сертификата**

Скрипт автоматически:
- Получит SSL сертификат через Let's Encrypt
- Настроит Nginx с вашим доменом
- Запустит Nginx контейнер

### Управление Nginx

После настройки вы можете управлять Nginx через меню скрипта `generate_ssl.sh`:

- **[3] 🌐 Запуск nginx** - Запустить контейнер
- **[4] 🛑 Остановка nginx** - Остановить контейнер
- **[5] 🔄 Перезапуск nginx** - Перезапустить контейнер
- **[6] 📊 Статус сервисов** - Показать статус
- **[7] 📋 Логи nginx** - Просмотр логов

### Подробная документация

Полная документация по настройке Nginx находится в файле `nginx/README.md`.

**Важно:** Nginx настраивается отдельно от основного скрипта установки UnicChat и является опциональным компонентом. UnicChat может работать напрямую на порту 8080 без Nginx.

<!-- TOC --><a name="-3-"></a>
## 3. Создание пользователя-администратора
1. При первом запуске откроется форма создания администратора:
   ![](./assets/form-setup-wizard.png "Форма создания администратора")
   * `Organization ID` - Идентификатор вашей организации, используется для подключения к push-серверу. Для получения ID необходимо написать запрос с указанием значения в Organization Name на почту support@unic.chat;
   * `Full name` - Имя пользователя, которое будет отображаться в чате;
   * `Username` - Логин пользователя, который вы будете указывать для авторизации;
   * `Email` - Действующая почта, используется для восстановления доступа;
   * `Password` - Пароль вашего пользователя;
   * `Confirm your password` - Подтверждение пароля;
2. После создания пользователя авторизуйтесь в веб-интерфейсе с использованием ранее указанных параметров.
3. Для включения пушей перейдите в раздел Администрирование - Push. Включите использование шлюза и укажите адрес шлюза `https://push1.unic.chat`.
4. Перейдите в раздел Администрирование - Organization, убедитесь, что поля заполнены в соответствии с п.1.
5. Настройка завершена.

<!-- TOC --><a name="-4-"></a>
## 4. Карта сетевых взаимодействий сервера

<!-- TOC --><a name="-unicchat-1"></a>
#### Входящие соединения на стороне сервера UnicChat:
Открыть порты:
- 8080/TCP - порт приложения UnicChat (используется для доступа к веб-интерфейсу);
- 27017/TCP - порт MongoDB (рекомендуется ограничить доступ только для внутренней сети);

<!-- TOC --><a name="-unicchat-2"></a>
#### Исходящие соединения на стороне сервера UnicChat:
* Открыть доступ для Push-шлюза:
  * 443/TCP, на хост `push1.unic.chat`;
* Открыть доступ для ВКС-сервера:
  * 443/TCP, на хост `lk-yc.unic.chat`;
  * 7880/TCP, 7881/TCP, 7882/UDP;
  * 5349/TCP, 3478/UDP;
  * (50000 - 60000)/UDP (диапазон этих портов может быть изменён при развертывании лицензионной версии непосредственно владельцем лицензии);
* Открыть доступ до внутренних ресурсов: LDAP, SMTP, DNS при необходимости использования этого функционала.

<!-- TOC --><a name="--5"></a>
## Частые проблемы при установке
Раздел в наполнении.

<!-- TOC --><a name="--6"></a>
## Клиентские приложения
* [Репозитории клиентских приложений]
* Android: (https://play.google.com/store/apps/details?id=pro.unicomm.unic.chat&pcampaignid=web_share)
* iOS: (https://apps.apple.com/ru/app/unicchat/id1665533885)
* Desktop: (https://github.com/unicommorg/unic.chat.desktop.releases/releases)
